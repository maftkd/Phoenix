...
public class Follow : MonoBehaviour
{
    ...
    void Update()
    {
		...
		if(_checkTimer>=_checkInterval){
			...
			if(diff.sqrMagnitude>_followDistance*_followDistance){
				//_bird.WaddleTo(_mate.transform.position+diff.normalized*_followDistance,1f);
				_bird.WaddleTo(_mate.transform.position-_mate.transform.forward*_followDistance,_followSpeed);
			}
			else
				_bird.StopWaddling();
		}
	...
	public void StartFollowingMate(){
		Debug.Log("We following "+_mate.name);
		enabled=true;
	}
